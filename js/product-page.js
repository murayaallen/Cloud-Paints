...
(function () {
  ...
  function init() {
    // A generated page under /paints/<slug> states its own slug, so no query
    // string is needed. The ?p= / ?slug= forms stay supported: old links and
    // bookmarks still land somewhere sensible while the 301s do their work.
    var slug = window.CP_PRODUCT_SLUG || param('p') || param('slug') || 'weatherguard';
    ...
    root.innerHTML =
      '<div style="min-height:70vh;display:flex;align-items:center;justify-content:center;text-align:center;padding:140px 20px;">' +
        '<div>' +
          ...
          '<a href="/products" class="btn btn-dark">Back to the catalogue</a>' +
        '</div>' +
      '</div>';
  }
  ...
  function applyThemeVars(p) {
    ...
    if (stage) {
      stage.style.setProperty('--stage-grad', 'linear-gradient(160deg, ' + (p.primary || '#b8243a') + ' 0%, ' + (p.accent || p.primary || '#d92843') + ' 100%)');
      // A tin is a cut-out that floats on the gradient; a texture is a
      // photograph of a finished wall, which has to fill the frame. Floated
      // at the tin's 78% it left the gradient showing down both sides and
      // read as a broken image. Same distinction the print package makes.
      stage.classList.toggle('pd-stage--tex', p.cat === 'texture');
    }
  }

  function writeHead(p) {
    document.title = p.name + ' — Cloud Paints';
    var desc = (p.short || p.full || p.name) + ' · ' + p.cat_label + '.';
    var m = document.querySelector('meta[name="description"]');
    if (m) m.setAttribute('content', desc);

    var pageUrl = 'https://cloudpaints.co.ke/paints/' + p.slug;
  ...
  function populate(p) {
    // A page generated by build/site.mjs already carries a title, a trimmed
    // description, a canonical, the social tags and full Product +
    // BreadcrumbList markup — written into the HTML, where a crawler sees
    // them without executing anything. Rewriting all of that from here would
    // only replace it with a worse version: the runtime description is not
    // length-trimmed, which is how twenty-two product pages ended up with
    // descriptions Google would cut mid-word.
    //
    // The block below is what keeps /product.html?p=<slug> working for old
    // links that have not yet followed their 301.
    if (!window.CP_PRODUCT_SLUG) writeHead(p);
    ...
    if (img) {
      // build/site.mjs walks the image folders and records what is actually
      // on disk. Sixteen of the twenty-eight products have no cut-out, and
      ...
      function typographicPanel() {
        ...
        if (!wrap || wrap.querySelector('.pd-stage-noart')) return;
        var fallback = document.createElement('div');
        fallback.className = 'pd-stage-noart';
      ...
      if (art) {
        img.alt = p.cat === 'texture'
          ? p.name + ' finish applied to a wall'
          : p.name + ' ' + (p.sizes && p.sizes[1] || '4L') + ' bucket';
        img.onerror = typographicPanel;
        img.src = art;
      } else {
